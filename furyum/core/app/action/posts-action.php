...
if(isset($_GET["opt"]) && $_GET["opt"]=="add"){
	...
	if(isset(Core::$user)) {
		$postAuthorName = !empty(Core::$user->username)
			? Core::$user->username
			: trim(Core::$user->name . ' ' . Core::$user->lastname);
		$postAuthorEmail = Core::$user->email;
	} else {
		$postAuthorName = "Anonimo";
		$postAuthorEmail = null;
	}
	$spamResult = Core::checkSpam($postAuthorName, $user->title . " " . $user->content, $postAuthorEmail);
	if($spamResult['isSpam']) {
		$user->status = 2; // Hidden status
		$user->add();
