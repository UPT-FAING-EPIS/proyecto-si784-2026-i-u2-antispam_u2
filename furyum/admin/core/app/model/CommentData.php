...
class CommentData {
	...
	public function add(){
		...
		$spam_reason = $this->spam_reason ? mysqli_real_escape_string($con, $this->spam_reason) : '';
		$ip_address = $this->ip_address ? mysqli_real_escape_string($con, $this->ip_address) : '';
		$sql = "insert into ".self::$tablename." (user_id,comment,post_id,status,spam_reason,ip_address,created_at) ";
		$sql .= "value (\"$this->user_id\",\"$comment\",$this->post_id,$this->status,\"$spam_reason\",\"$ip_address\", NOW())";
...
// partiendo de que ya tenemos creado un objecto CommentData previamente utilizamos el contexto
	...
	public function accept(){
		$sql = "update ".self::$tablename." set status=2, spam_reason=NULL where id=$this->id";
		Executor::doit($sql);
	}
